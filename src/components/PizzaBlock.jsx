      ...
      <div className="pizza-block__selector">
        <ul>
          {types.map((typeId, index) => (
            <li className={activeType === index ? "active" : ""} onClick={() => setActiveType(index)} key={index}>{typeNames[typeId]}</li>
          ))}
        </ul>
        <ul>
          {sizes.map((sizeId, index) => (
            <li className={activeSize === index ? "active" : ""} onClick={() => setActiveSize(index)} key={index}>{sizeId} см.</li>
          ))}
        </ul>
      </div>
